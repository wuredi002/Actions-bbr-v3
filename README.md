...
脚本会优先安装并运行 Ookla 官方 `speedtest`，让用户选择中国移动、中国联通、中国电信或自动匹配三大运营商，然后优先尝试中国境内最近的匹配测速节点并获取上传带宽、下载带宽和 Ping；如果找不到匹配节点，会降级为中国境内不限运营商节点，最后才使用 Ookla 自动选择。测速失败时会提示手动输入上传带宽。

优化逻辑：

- 自动启用 `bbr` 拥塞控制和 `fq` 队列算法。
- 测速节点优先选择中国境内节点，并支持三大运营商选择。
